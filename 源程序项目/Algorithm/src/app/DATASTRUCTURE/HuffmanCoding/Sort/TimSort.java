/*
 * @Author: 孔潇
 * @Date: 2020-06-30 17:56:08
 * @LastEditTime: 2020-06-30 19:49:46
 * @Description:  MyTimSort实例
...
public class TimSort<T> {
     ...
     * <p/>
     * 这个常量的值必须2的幂。Tim Perter
     * 在C语言中的实现版本使用了64，但是根据经验这里的版本使用32更合适。在最坏的情况下，使用了非2的幂赋值，就必须要重写
     * {@link # minRunLength}这个方法。 如果减小了这个值，就需要在构造方法中减小stackLen的值，不然将面临数组越界的风险。
     */
     ...
     * 下面这个根据前一个run的结尾总是下一个run的开头。 所以下面的等式总是成立: runBase[i] + runLen[i] ==
     * runBase[i+1];
     **/
    ...
    /**
     * 这个构造方法是私有的所以只能在类内部创建。 创建这个实例是为了保存一次排序过程中的状态变量。
     */
    private TimSort(T[] a, Comparator<? super T> c) {
        ...
        /**
         * 这里是分配储存run的栈的空间，它不能在运行时扩展。 C语言版本中的栈一直使用固定值85，但这样对一些中小数组来说有些浪费资源。所以，
         * 这个版本我们使用了相对较小容量的栈。 在MIN_MERGE减小的时候，这些‘魔法数’可能面临数组越界的风险。
         */
     ...
     * 被优化的二分插入排序
     *
     * 使用二分插入排序算法给指定一部分数组排序。这是给小数组排序的最佳方案。最差情况下 它需要 O(n log n) 次比较和 O(n^2)次数据移动。
     *
     * 如果开始的部分数据是有序的那么我们可以利用它们。这个方法默认数组中的位置lo(包括在内)到 start(不包括在内)的范围内是已经排好序的。
     *
     * @param a     被排序的数组
    ...
    /**
     * 这一段代码是TimSort算法中的一个小优化，它利用了数组中前面一段已有的顺序。 如果是升序，直接返回统计结果；如果是降序，在返回之前，将这段数列倒置，
     * 以确保这断序列从首个位置到此位置的序列都是升序的。 返回的结果是这种两种形式的，lo是这段序列的开始位置。
     *
     ...
     * a[lo] > a[lo + 1] > a[lo + 2] > ...
     *
     * 为了保证排序的稳定性，这里要使用严格的降序，这样才能保证相等的元素不参与倒置子序列的过程， 保证它们原本的顺序不被打乱。
     *
     * @param a  参与排序的数组
    ...
    /**
     * 返回参与合并的最小长度，如果自然排序的长度，小于此长度，那么就通过二分查找排序扩展到 此长度。{@link #binarySort}.
     *
     * 粗略的讲，计算结果是这样的：
     *
     * 如果 n < MIN_MERGE, 直接返回 n。（太小了，不值得做复杂的操作）； 如果 n 正好是2的幂，返回 n / 2； 其它情况下 返回一个数
     * k，满足 MIN_MERGE/2 <= k <= MIN_MERGE, 这样结果就能保证 n/k 非常接近但小于一个2的幂。
     * 这个数字实际上是一种空间与时间的优化。
     *
     * @param n 参与排序的数组的长度
     * @return 参与归并的最短长度 这段代码写得也很赞
     */
    ...
    /**
     * Pushes the specified run onto the pending-run stack. 将指定的升序序列压入等待合并的栈中
     *
    ...
    /**
     * 检查栈中待归并的升序序列，如果他们不满足下列条件就把相邻的两个序列合并， 直到他们满足下面的条件
     *
     * 1. runLen[i - 3] > runLen[i - 2] + runLen[i - 1] 2. runLen[i - 2] > runLen[i
     * - 1]
     *
     * 每次添加新序列到栈中的时候都会执行一次这个操作。所以栈中的需要满足的条件 需要靠调用这个方法来维护。
     *
     * 最差情况下，有点像玩2048。
     */
    ...
    /**
     * 在一个序列中，将一个指定的key，从左往右查找它应当插入的位置；如果序列中存在 与key相同的值(一个或者多个)，那返回这些值中最左边的位置。
     *
     * 推断： 统计概率的原因，随机数字来说，两个待合并的序列的尾假设是差不多大的，从尾开始 做查找找到的概率高一些。仔细算一下，最差情况下，这种查找也是
     * log(n)，所以这里没有 用简单的二分查找。
     *
     * @param key  准备插入的key
     * @param a    参与排序的数组
     ...
     * @param c    排序，查找使用的比较器
     * @return 返回一个整数 k, 有 0 <= k <=n, 它满足 a[b + k - 1] < a[b + k] 就是说key应当被放在
     *         a[base + k], 有 a[base,base+k) < key && key <=a [base + k, base + len)
     */
    ...
    /**
     * 与gallopLeft相似，不同的是如果发现key的值与某些元素相等，那返回这些值最后一个元素的位置的 后一个位置
     *
    ...
    /**
     * 合并在栈中位于i和i+1的两个相邻的升序序列。 i必须为从栈顶数，第二和第三个元素。 换句话说i == stackSize - 2 || i ==
     * stackSize - 3
     *
    ...
    private void mergeAt(int i) {
         ...
         * 往前移一位，因为本次合并不关它的事。i+1对应的序列被合并到i序列中了，所以 i+1 数列可以消失了
         */
        runLen[i] = len1 + len2;
        if (i == stackSize - 3) {
    ...
    /**
     * 使用固定空间合并两个相邻的有序序列，保持数组的稳定性。 使用本方法之前保证第一个序列的首个元素大于第二个序列的首个元素；第一个序列的末尾元素
     * 大于第二个序列的所有元素
     *
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
        ...
        outer: while (true) {
            /*
             * 这里加了两个值来记录一个序列连续比另外一个大的次数，根据此信息，可以做出一些 优化
             */
            ...
            /*
             * 执行到这里的话，一个序列会连续的的比另一个序列大，那么这种连续性可能持续的 更长。那么我们就按照这个逻辑试一试。直到这种连续性被打破。根据找到的长度，
    ...
    /**
     * 保证临时数组的大小能够容纳所有的临时元素，在需要的时候要扩展临时数组的大小。 数组的大小程指数增长，来保证线性的复杂度。
     *
     * 一次申请步长太小，申请的次数必然会增多，浪费时间；一次申请的空间足够大，必然会 浪费空间。正常情况下，归并排序的临时空间每次大的合并都会 * 2，
